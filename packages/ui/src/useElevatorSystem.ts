...
const DEFAULT_EXECUTION_TIME = getDefaultVariableAsInt(
  process.env.REACT_APP_EXECUTION_TIME,
);
...
export const useElevatorSystem = () => {
  ...
  const handleCallElevator = (): void => {
    if (!currentFloor || !targetFloor || !building.system || !floors) {
      return;
    }
  ...
  const handleRemoveElevatorCar = useCallback(
    (id: number) => {
      if (!building.system) {
        return;
      }
